...
nfx fibonacciclock [Hourcolor] [MinuteColor] [BothColor] [UnusedColor set "off" to disable] [Num LED Box 1] [Num LED Box 12]  [Num LED Box 2]  [Num LED Box 3]  [Num LED Box 5]
...
enum modetype {
  Off, On, Fade, ColorFade, Rainbow, Kitt, Comet, Theatre, Scan, Dualscan, Twinkle, TwinkleFade, Sparkle, Fire, FireFlicker, Wipe, Dualwipe, FakeTV, SimpleClock,FibonacciClock
};
...
//          for (int i = 0; i < pixelCount ; i = i + (pixelCount / parseString(string, 3).toInt())) {
        ...
        else if (subCommand == F("fibonacciclock")) {
          mode = FibonacciClock;
          ...
          rgb_h = (parseString(string, 3) == "") ? RgbColor(255,10,10) : RgbColor ( rgbStr2Num(parseString(string, 3)) >> 16, rgbStr2Num(parseString(string, 3)) >> 8, rgbStr2Num(parseString(string, 3)));
          rgb_m = (parseString(string, 4) == "") ? RgbColor(10,255,10) : RgbColor ( rgbStr2Num(parseString(string, 4)) >> 16, rgbStr2Num(parseString(string, 4)) >> 8, rgbStr2Num(parseString(string, 4)));
          rgb_b = (parseString(string, 5) == "") ? RgbColor(10,10,255) : RgbColor ( rgbStr2Num(parseString(string, 5)) >> 16, rgbStr2Num(parseString(string, 5)) >> 8, rgbStr2Num(parseString(string, 5)));

          if (parseString(string, 6) != "")
          {
            if (parseString(string, 6) == "off") {
              rgb_s_off = true;
              rgb_s = RgbColor(0,0,0);
            }
            else
            {
              rgb_s_off = false;
              rgb_s = RgbColor ( rgbStr2Num(parseString(string, 6)) >> 16, rgbStr2Num(parseString(string, 6)) >> 8, rgbStr2Num(parseString(string, 6)));
            }
          }
          else
          {
            rgb_s = RgbColor(255,255,255);
          }

          FibonacciBox1Leds  = (parseString(string,  7) == "") ? 1 : parseString(string, 7).toInt();
          FibonacciBox12Leds = (parseString(string,  8) == "") ? 1 : parseString(string, 8).toInt();
          FibonacciBox2Leds  = (parseString(string,  9) == "") ? 1 : parseString(string, 9).toInt();
          FibonacciBox3Leds  = (parseString(string, 10) == "") ? 1 : parseString(string, 10).toInt();
          FibonacciBox5Leds  = (parseString(string, 11) == "") ? 1 : parseString(string, 11).toInt();


          if ((FibonacciBox1Leds +FibonacciBox12Leds + FibonacciBox2Leds + FibonacciBox3Leds + FibonacciBox5Leds) > pixelCount)
          {
            FibonacciBox1Leds  = 1;
            FibonacciBox12Leds = 1;
            FibonacciBox2Leds  = 1;
    ...
    case PLUGIN_CLOCK_IN:
      {
        if (mode == FibonacciClock)
        {
          Plugin_124_fibonacciclock();
        }
...
void Plugin_124_fibonacciclock()
{
  byte hours = hour();
  byte minutes = minute();

  TimeConverter tc(hours, minutes);
  FibonacciValues fs = tc.getFibonacciTime();

  rrggbb = RgbColor(0, 0, 0);

  for ( int i = 1; i <= 5; i++){
    bool hourUsed = fs.getFieldHour(i);
    bool minuteUsed = fs.getFieldMinute(i);

    if (hourUsed && minuteUsed)
    {
      rrggbb = rgb_b;
    }
    else if(hourUsed)
    ...
    Plugin_124_fibonacciclockSetBox(i);
  }
}


void Plugin_124_fibonacciclockSetBox(int box)
{
  int offset = 0;
  int count = 1;
  ...
  switch(box)
  {
    case 1: // 1 5
      offset = 0;
      count = FibonacciBox1Leds;
    break;

    case 2: // 1.2 5
        offset = FibonacciBox1Leds;
        count = offset + FibonacciBox12Leds;
    break;

    case 3: // 2 10
      offset = FibonacciBox1Leds + FibonacciBox12Leds;
      count = offset + FibonacciBox2Leds;
    break;

    case 4: // 3 15
      offset = FibonacciBox1Leds + FibonacciBox12Leds + FibonacciBox2Leds;
      count = offset + FibonacciBox3Leds;
    break;

    case 5: // 5 25
      offset = FibonacciBox1Leds + FibonacciBox12Leds + FibonacciBox2Leds + FibonacciBox3Leds;
      count = offset + FibonacciBox5Leds;
    break;
  }
